...
- **Driver/TD market shortlisting REQUIRES every sourced attribute floor at once** (explicit user decision 2026-08-10, overriding the earlier Newbie-Guide "one or two skills will suffice" behaviour). `mkFullStatsTable` gates on ALL attributes with a numeric minimum (min > 0): a candidate must clear every floor to be "meets all minimums", and candidates that miss any floor are shown separately with the specific floor(s) they failed. `"0-xx"` keep-low targets parse to a 0 floor and impose no lower bound, so they never block. If nothing passes, the panel tells the user the thresholds are likely too high — that's expected/correct, not a bug. Keep `currentLeague`-driven per-league floors in mind when editing numbers.
